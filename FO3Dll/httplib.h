...
inline EncodingType encoding_type(const Request &req, const Response &res) {
  auto ret =
      detail::can_compress_content_type(res.get_header_value(xorstr("Content-Type")));
...
bool prepare_content_receiver(T &x, int &status, ContentReceiver receiver,
  ...
  if (decompress) {
    std::string encoding = x.get_header_value(xorstr("Content-Encoding"));
...
bool read_content(Stream &strm, T &x, size_t payload_max_length, int &status,
  ...
  return prepare_content_receiver(
      x, status, receiver, decompress, [&](const ContentReceiver &out) {
        ...
        } else if (!has_header(x.headers, xorstr("Content-Length"))) {
          ret = read_content_without_length(strm, out);
        } else {
          auto len = get_header_value<uint64_t>(x.headers, xorstr("Content-Length"));
...
inline bool redirect(T &cli, const Request &req, Response &res,
  ...
  if (res.status == 303 && (req.method != xorstr("GET") && req.method != xorstr("HEAD"))) {
    new_req.method = xorstr("GET");
    new_req.body.clear();
...
inline std::string make_multipart_data_boundary() {
  ...
  std::string result = xorstr("--cpp-httplib-multipart-data-");
...
inline bool expect_content(const Request &req) {
  if (req.method == xorstr("POST") || req.method == xorstr("PUT") || req.method == xorstr("PATCH") ||
      req.method == xorstr("PRI") || req.method == xorstr("DELETE")) {
    return true;
  }
...
inline std::pair<std::string, std::string> make_digest_authentication_header(
  ...
  auto key = is_proxy ? xorstr("Proxy-Authorization") : xorstr("Authorization");
...
inline bool parse_www_authenticate(const Response &res,
                                   ...
                                   bool is_proxy) {
  auto auth_key = is_proxy ? xorstr("Proxy-Authenticate") : xorstr("WWW-Authenticate");
...
inline bool Request::is_multipart_form_data() const {
  const auto &content_type = get_header_value(xorstr("Content-Type"));
...
inline void Response::set_content(const char *s, size_t n,
                                  const char *content_type) {
  body.assign(s, n);
  set_header(xorstr("Content-Type"), content_type);
}
...
inline bool Server::parse_request_line(const char *s, Request &req) {
  const static std::regex re(
      xorstr("(GET|HEAD|POST|PUT|DELETE|CONNECT|OPTIONS|TRACE|PATCH|PRI) (([^?]+)(?:\\?(.*?))?) (HTTP/1\\.[01])\r\n"));
...
inline bool Server::write_response(Stream &strm, bool close_connection,
  ...
  // Response line
  if (!bstrm.write_format(xorstr("HTTP/1.1 %d %s\r\n"), res.status,
                          detail::status_message(res.status))) {
    return false;
  }

  // Headers
  if (close_connection || req.get_header_value(xorstr("Connection")) == xorstr("close")) {
    res.set_header(xorstr("Connection"), xorstr("close"));
  } else {
    std::stringstream ss;
  ...
  if (!res.has_header(xorstr("Content-Type")) &&
      (!res.body.empty() || res.content_length_ > 0 || res.content_provider_)) {
    res.set_header(xorstr("Content-Type"), xorstr("text/plain"));
  }

  if (!res.has_header("Accept-Ranges") && req.method == xorstr("HEAD")) {
  ...
  if (req.ranges.size() > 1) {
    ...
    auto it = res.headers.find(xorstr("Content-Type"));
    if (it != res.headers.end()) {
      content_type = it->second;
      res.headers.erase(it);
    }

    res.headers.emplace(xorstr("Content-Type"),
  ...
  if (res.body.empty()) {
    if (res.content_length_ > 0) {
      ...
      if (req.ranges.empty()) {
        ...
        res.set_header(xorstr("Content-Range"), content_range);
      } else {
        length = detail::get_multipart_ranges_data_length(req, res, boundary,
                                                          content_type);
      }
      res.set_header(xorstr("Content-Length"), std::to_string(length));
    } else {
      if (res.content_provider_) {
        if (res.is_chunked_content_provider) {
          res.set_header("Transfer-Encoding", "chunked");
          if (type == detail::EncodingType::Gzip) {
            res.set_header(xorstr("Content-Encoding"), "gzip");
          } else if (type == detail::EncodingType::Brotli) {
            res.set_header(xorstr("Content-Encoding"), "br");
          }
        }
      } else {
        res.set_header(xorstr("Content-Length"), "0");
      }
    }
  } else {
    if (req.ranges.empty()) {
    ...
    res.set_header(xorstr("Content-Length"), length);
  }
...
inline bool Server::read_content(Stream &strm, Request &req, Response &res) {
  ...
  if (read_content_core(
    ...
    const auto &content_type = req.get_header_value(xorstr("Content-Type"));
...
inline bool Server::read_content_core(Stream &strm, Request &req, Response &res,
  ...
  if (req.method == "DELETE" && !req.has_header(xorstr("Content-Length"))) {
    return true;
  }
    ...
    if (!req.path.compare(0, mount_point.size(), mount_point)) {
      ...
      if (detail::is_valid_path(sub_path)) {
        ...
        if (detail::is_file(path)) {
          ...
          if (type) { res.set_header(xorstr("Content-Type"), type); }
          res.status = 200;
...
inline bool Server::routing(Request &req, Response &res, Stream &strm) {
  // File handler
  bool is_head_request = req.method == xorstr("HEAD");
  if ((req.method == xorstr("GET") || is_head_request) &&
  ...
  // Regular handler
  if (req.method == xorstr("GET") || req.method == xorstr("HEAD")) {
    return dispatch_request(req, res, get_handlers_);
  } else if (req.method == xorstr("POST")) {
    return dispatch_request(req, res, post_handlers_);
  } else if (req.method == xorstr("PUT")) {
    return dispatch_request(req, res, put_handlers_);
  } else if (req.method == xorstr("DELETE")) {
    return dispatch_request(req, res, delete_handlers_);
  } else if (req.method == xorstr("OPTIONS")) {
...
Server::process_request(Stream &strm, bool close_connection,
  ...
  if (req.get_header_value(xorstr("Connection")) == "close") {
    connection_closed = true;
  }

  if (req.version == xorstr("HTTP/1.0") &&
      req.get_header_value(xorstr("Connection")) != "Keep-Alive") {
    connection_closed = true;
  }
  ...
  if (req.get_header_value("Expect") == "100-continue") {
    ...
    case 100:
    case 417:
      strm.write_format(xorstr("HTTP/1.1 %d %s\r\n\r\n"), status,
...
inline bool ClientImpl::read_response_line(Stream &strm, Response &res) {
  ...
  const static std::regex re(xorstr("(HTTP/1\\.[01]) (\\d+) (.*?)\r\n"));
...
inline bool ClientImpl::handle_request(Stream &strm, const Request &req,
  ...
  if (!is_ssl() && !proxy_host_.empty() && proxy_port_ != -1) {
    auto req2 = req;
    req2.path = xorstr("http://") + host_and_port_ + req.path;
...
inline bool ClientImpl::redirect(const Request &req, Response &res) {
  ...
  auto scheme = is_ssl() ? xorstr("https") : xorstr("http");
  ...
  if (!port_str.empty()) {
    ...
    next_port = next_scheme == xorstr("https") ? 443 : 80;
  }
...
inline bool ClientImpl::write_request(Stream &strm, const Request &req,
  ...
  bstrm.write_format(xorstr("%s %s HTTP/1.1\r\n"), req.method.c_str(), path.c_str());

  // Additonal headers
  Headers headers;
  if (close_connection) { headers.emplace(xorstr("Connection"), "close"); }
  ...
  if (!req.has_header(xorstr("User-Agent"))) {
    headers.emplace(xorstr("User-Agent"), xorstr("cpp-httplib/0.7"));
  }

  if (req.body.empty()) {
    if (req.content_provider) {
      auto length = std::to_string(req.content_length);
      headers.emplace(xorstr("Content-Length"), length);
    } else {
      headers.emplace(xorstr("Content-Length"), "0");
    }
  } else {
    if (!req.has_header(xorstr("Content-Type"))) {
      headers.emplace(xorstr("Content-Type"), xorstr("text/plain"));
    }

    if (!req.has_header(xorstr("Content-Length"))) {
...
inline std::shared_ptr<Response> ClientImpl::send_with_content_provider(
  ...
  if (content_type) { req.headers.emplace(xorstr("Content-Type"), content_type); }
...
inline bool ClientImpl::process_request(Stream &strm, const Request &req,
  ...
  // Body
  if (req.method != xorstr("HEAD") && req.method != xorstr("CONNECT")) {
    auto out =
  ...
  if (res.get_header_value(xorstr("Connection")) == "close" ||
      (res.version == xorstr("HTTP/1.0") && res.reason != xorstr("Connection established"))) {
    stop_core();
  }
...
inline Result ClientImpl::Post(const char *path, const Headers &headers,
                               ...
                               const char *content_type) {
  auto ret = send_with_content_provider(xorstr("POST"), path, headers, body, 0, nullptr,
                                        content_type);
...
inline Result ClientImpl::Post(const char *path, const Headers &headers,
                               ...
                               ContentProvider content_provider,
                               const char *content_type) {
  auto ret = send_with_content_provider(xorstr("POST"), path, headers, std::string(),
...
inline Result ClientImpl::Put(const char *path, const Headers &headers,
                              const std::string &body,
                              const char *content_type) {
  auto ret = send_with_content_provider(xorstr("PUT"), path, headers, body, 0, nullptr,
                                        content_type);
  return Result{ret, get_last_error()};
}
...
inline Result ClientImpl::Put(const char *path, const Headers &headers,
                              ...
                              ContentProvider content_provider,
                              const char *content_type) {
  auto ret = send_with_content_provider(xorstr("PUT"), path, headers, std::string(),
                                        content_length, content_provider,
                                        content_type);
  return Result{ret, get_last_error()};
}
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
    if (!scheme.empty() && (scheme != xorstr("http") && scheme != xorstr("https"))) {
#else
    if (!scheme.empty() && scheme != xorstr("http")) {
#endif
      return;
    }

    auto is_ssl = scheme == xorstr("https");
